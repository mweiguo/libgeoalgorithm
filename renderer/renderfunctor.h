...
inline void QtRenderFunctor::apply ( TextNode& text )
{
    ...
    // get lines in string
    int lines  = count (text.content().begin(), text.content().end(), '\n') + 1;
    // get height of all text
    int height = lines * m.lineSpacing ();
    // get longest width of each line text
    int width=0;
    if ( 1 == lines )
	width = m.boundingRect ( text.content().c_str() ).width();
    else {
	string::const_iterator pp, end=text.content().end(), lastpp=text.content().begin();
	for ( pp=text.content().begin(); pp != end; ++pp ) {
	    if ( *pp == '\n' )
		{
    ...
    _opt->painter->drawText ( b, text.alignFlag() | Qt::TextWordWrap | Qt::TextDontClip, text.content().c_str() );
