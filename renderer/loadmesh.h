...
inline LoadMesh::LoadMesh ( const char* fileName, bool needExpand, bool needSceneManagement )// : _opt(opt)
{
    int clo = clock();
    XercesParser parser;
...
inline void LoadMesh::traverseNode ( XERCES_CPP_NAMESPACE::DOMElement* pnode, SGNode* parent )
{
    ...
    for ( DOMElements::iterator pp=tagTexts.begin(); pp!=tagTexts.end(); ++pp ) {
        ...
        if ( XercesHelper::hasAttribute ( tagText, "anchor" ) )
            textnode->setAnchor ( atoi((const char*)XercesHelper::getAttribute ( tagText, "anchor" )));
